...
const fileList = fileData.findAllShareFile(email);
let html = '';
...
window.onload = () => {
  ...
  <i class="far fa-${type} mr-2 blue-text" aria-hidden="true"></i>${file.name}</div></th>`;
    ...
    html
      += '<i class="material-icons" style="font-size:18px">priority_high</i>';
  ...
  fileList.forEach((file) => {
    ...
    document
      ...
      .addEventListener('click', () => {
        const filePath = `/home/${user}/ipfsbox/${email}/${file.name}`;
        if (file.type === '.txt') {
    ...
    document.getElementById(file.version[file.version.length - 1].cid).addEventListener('click', () => {
      const precid = null;
      if (file.version.length > 1) precid = file.version[file.version.length - 2];
      const data = {
